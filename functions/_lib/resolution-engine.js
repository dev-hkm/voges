...
export function compileResolutionPlan({ input, decline, card, profile, kyc }) {
  ...
  const addCause = (cause) => {
    if (!cause || causeCodes.has(cause.code)) return;
    causeCodes.add(cause.code);
    causes.push(cause);
  };
  const addStep = (step) => {
    if (!step || stepTools.has(step.tool_name)) return;
    stepTools.add(step.tool_name);
    steps.push(step);
  };
  ...
  // The decline reason is only the first recorded blocker. Build the plan from
  // the whole current card state, then order dependencies. A locked card must
  // be unlocked before payment controls can be changed during plan execution.
  if (selectedCard?.status === 'locked') {
    addCause({ code: 'locked_card', label: CAUSE_COPY.locked_card, status: 'blocking', detail: 'The affected card is currently locked.' });
    addStep({ id: 'unlock-card', tool_name: 'unfreezeCard', title: 'Unlock card', description: 'Unlock the affected card after device verification.', payload: { card_id: selectedCard.id }, risk_level: 'high', estimated_effect: 'The card can be used for the remaining readiness checks.' });
  }
  if (selectedCard && !onlineEnabled) {
    addCause({ code: 'online_payment_disabled', label: CAUSE_COPY.online_payment_disabled, status: 'blocking', detail: 'Online payments are disabled for the affected card.' });
    addStep({ id: 'enable-online-payments', tool_name: 'enableOnlinePayments', title: 'Enable online payments', description: `Enable online payments for card ending ${String(selectedCard.masked_number || '').replace(/\D/g, '').slice(-4)}.`, payload: { card_id: selectedCard.id }, risk_level: 'medium', estimated_effect: 'Online merchant payments can proceed.' });
  }
  if (reason === 'daily_limit' && selectedCard) {
    const newLimit = nextDailyLimit(selectedCard, transaction.amount);
    if (newLimit) {
      addStep({ id: 'increase-daily-limit', tool_name: 'updateCardDailyLimit', title: 'Increase daily limit', description: `Increase the daily limit from ${selectedCard.daily_limit} to ${newLimit}.`, payload: { card_id: selectedCard.id, daily_limit: newLimit }, risk_level: 'high', estimated_effect: 'The verified payment amount fits within the daily limit.' });
    } else {
      addCause({ code: 'daily_limit_requires_review', label: 'The required limit change exceeds the permitted monthly limit', status: 'blocking', detail: 'A human review is required.' });
      addStep({ id: 'limit-review', tool_name: 'createSupportTicket', title: 'Request a limit review', description: 'Create a support request for a permitted card-limit change.', payload: { subject: 'Card limit review requested', description: 'Resolution Autopilot found a daily-limit decline that cannot be safely resolved within the current monthly limit.' }, risk_level: 'medium', estimated_effect: 'A support specialist can review the limit safely.' });
    }
  }
  if (reason === 'international_disabled' && selectedCard) {
    addStep({ id: 'enable-international-payments', tool_name: 'enableInternationalPayments', title: 'Enable international payments', description: 'Enable international payments for the affected card.', payload: { card_id: selectedCard.id }, risk_level: 'high', estimated_effect: 'The card can be used with international merchants.' });
  }
  if (['kyc_required', 'expired_card', 'suspicious_activity', 'fraud_detected', 'monthly_limit'].includes(reason)) {
    addStep({ id: 'create-support-ticket', tool_name: 'createSupportTicket', title: 'Create support ticket', description: 'Create a secure support request for the remaining verified blocker.', payload: { subject: `Resolution support requested: ${transaction.merchant_name || 'payment'}`, description: `Resolution Autopilot identified ${CAUSE_COPY[reason] || 'a verified payment restriction'}.` }, risk_level: 'medium', estimated_effect: 'A qualified support specialist can continue the resolution.' });
  }
